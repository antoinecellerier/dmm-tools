> [!WARNING]
> **This is an automated development build, not a release.** It is built from the latest commit on `main`, and may be broken. For normal use, download the [latest stable release](__REPO__/releases/latest).
...
Please include the output of `dmm-cli --version` — on this build it ends with `(__SHA__)` — so the report points at the exact commit it came from. [Open an issue](__REPO__/issues/new).
