...
impl Protocol for Ut8803Protocol {
    ...
    fn capture_steps(&self) -> Vec<crate::protocol::CaptureStep> {
        ...
        // The dial legends this meter prints ("DCV", "OHM") go in the two
        // "turn the dial" steps; the rest of the gate is the shared wording.
        let [dcv, dcv_short, dcv_negative, ohm, ohm_body, ohm_short] = steps::gate_steps(
            ...
            CaptureStep::basic(
                "ohm",
                "Set meter to Resistance (OHM). Leave leads open (should show OL).",
            ),
        );
