...
| `protocol/framing.rs` | Message framing: find `AB CD` or `0xAC`, extract payload, validate checksum (or position validation); build the `AB CD` command frame the UT61+ and Voltcraft families send |
| `protocol/ut61eplus/` | UT61E+ family: `Ut61PlusProtocol`, `Mode` enum, `Command` enum, `tables/` (per-model `ModeTables` impls — one match per mode returning ranges and, for the UT61B+/UT61D+, specs — behind the `DeviceTable` trait), `specs/` (the UT61E+'s manual spec tables, which `SpecModel` looks a reading up in) |
...
| `protocol/ut80x/` | UT803/UT804: `Ut80xProtocol` — streaming, CR LF packets (CH9325 HID) |
...
13. **Static spec data** — per-range specifications (resolution, accuracy bands) and per-mode metadata (input impedance, notes) are `&'static` data in `specs_*.rs` files, transcribed from device manuals — positional arrays in the UT61B+/UT61D+ `tables/`, rows keyed by range byte (`ModeSpecs`) elsewhere. `Dmm::request_measurement` attaches them to each reading through `Protocol::spec_info(&Measurement)` / `mode_spec_info(&Measurement)`, which take the whole reading so a family can pick the table by more than its mode and range. Use `cargo run -p dmm-lib --example dump_specs` to verify spec data against manuals.
