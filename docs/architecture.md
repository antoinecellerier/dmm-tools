...
16. **Session clock** — `dmm_lib::Clock` is the time base a session's readings are stamped with, cloned into the `Dmm`, the mock's waveform and the pacing loop so all three read the same time. `Clock::real()` is wall time and what production runs on. `Clock::scaled(f)` runs session time at `f` times real time, and `with_preseed(secs)` adds a burst the pacing sleeps spend instantly, so a session can start with minutes of history in a few milliseconds — screenshots and performance runs stop costing the time they show. `Clock::manual()` only moves when a test advances it, which is how the pacing and mock time-travel tests assert virtual deltas without sleeping. `Dmm::request_measurement` is the single stamping point: it overwrites the `Instant::now()` the parsers set, so stats, graph, recording and export follow without knowing a clock exists, and `WallClock::from_clock` backdates its system origin by the burst so exported wall times stay true. Hardware-facing timing stays real — transport bring-up sleeps, frame read deadlines, the UT181A reply deadline, VC890 ack gaps and the cycle driver's settle delay pace physical USB, which no clock can hurry.
