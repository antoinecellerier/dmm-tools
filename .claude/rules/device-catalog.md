...
- Status is the registry's `Stability::label()` and nothing else:
  `✅ Verified`, `🟡 Partly verified` or `🧪 Experimental`, the last two with
  the verification issue linked. The README's device table uses the same
  words and emoji.
