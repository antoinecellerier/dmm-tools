...
- Popups and dropdowns (`ComboBox`, `Popup::menu`) leave keyboard focus on the opener, so an opened popup is unreachable without Tab and Tab walks out of it. Every popup must move focus into itself on the frame it opens, handle Arrow/Enter/Esc, close on Tab, and return focus to the opener on close — `color_edit` in `app/controls.rs` and `show_dropdown` in `display.rs` are the pattern.
- egui wraps id salts in `IdSalt`, so code that reproduces a widget's id must do the same: `make_persistent_id(IdSalt::new(salt))` matches `ComboBox::from_id_salt`; `Id::new(salt)` or the bare `&str` give a different id and nothing fails at compile time — `show_dropdown` in `display.rs` is the example.
...
- Programmatic scrolls (`scroll_to_me`, `scroll_with_delta`) run over egui's `scroll_animation` and land a few frames later, so a headless test that reads the offset right after the frame catches it mid-flight — advance `RawInput::time` by a second per frame, as the settings and shortcut-help harnesses do. Keep the animation: it is the same one the wheel and every egui app use.
- A `ui.horizontal` inside `horizontal_wrapped` never wraps: it claims the rest of the row and its content overflows the panel, and `Region::expand_to_include_rect` widens the parent's `max_rect` with it, so every wrapped row in the panel keeps folding at the overflowed width after the window narrows. Size a composite unit up front (`WidgetText::into_galley` for the text) and place it with `ui.allocate_ui_with_layout(size, ..)`, as `color_edit` in `app/controls.rs` does.
...
- `Area::anchor` offsets from `ctx.content_rect()`, not from the space the panels left over, so an overlay meant to sit under a panel has to add that panel's height to the offset itself — the toast passes the top panel's `response.rect.bottom()` down for exactly this.
- `Fonts::has_glyph` is a false negative for every character in the face that owns epaint's replacement character `◻` — the emoji font in the default set, so `⚠` and `✔` report missing while drawing fine. To test that a glyph exists, lay it out and compare its `uv_rect` with the replacement character's, as `the_glyphs_have_fonts` in `app/toast.rs` does. And check the bundled fonts before picking a symbol: `✓` (U+2713) has no glyph anywhere and draws as a box; `✔` (U+2714) does.
