...
- **Brief overloads now break the graph line instead of being drawn through** — an over-range excursion shorter than the gap threshold left no hole in the timestamps, so the trace ran straight from the last good reading to the first one after it, and the visible-range integral counted the area under that invented line. Overloads now split the trace and mark the gap, with the opening marker appearing as soon as the meter goes over range rather than only once it recovers.
...
- **UT8802 readings now carry the right unit magnitude** — the meter's digits are relative to the selected range, but the unit was reported without the range's prefix, so a 10 nF capacitor logged as "10.00 F" and a 1.5 MHz signal as "1.500 Hz" (out by up to a factor of 10⁹). Resistance above 2 kΩ, and the 200 mV range, were wrong the same way. Units now read nF/µF/mF, Hz/kHz/MHz, Ω/kΩ/MΩ and mV.
- **Export CSV explains itself when there is nothing to export** — clicking it (or pressing Ctrl+E) before recording did nothing at all: no dialog, no message, no log line. It now says the buffer is empty.
- **The reconnect loop can now be cancelled** — while retrying, the tooltip said "click Disconnect to stop" but no Disconnect button was shown and `Ctrl+Shift+C` did nothing, so unplugging the meter left the GUI retrying every two seconds with no way out but quitting. Both now work.
- **The graph no longer goes blank after changing mode while zoomed** — a Y range pinned by a Shift-drag zoom stayed pinned when the dial moved, so the new mode's trace sat far outside the plot with the previous mode's numbers still on the axis, and nothing said why. Changing mode now releases the zoom, as Clear and Reset Zoom already did.
- **The right USB cable is picked when several are plugged in** — the adapter was chosen by a fixed CP2110 → CH9329 → CH9325 order regardless of which meter you selected, so a UT803 on a bench that also had a UT61E+ attached opened the wrong cable and reported "No response from meter". Selection now prefers the cable the chosen meter ships with, still falling back to the others.
- **Meters that share a protocol now report their own model** — selecting UT161E showed "UNI-T UT61E+" and, worse, presented it as hardware-verified: no EXPERIMENTAL badge, no prompt to submit a capture, and the feedback link pointed at the generic issues page. UT161B, UT161D and the Voltcraft VC650BT were mislabelled the same way. All of them now identify correctly and are flagged as unverified.
- **The meter's high-voltage warning now shows in the GUI** — the HV indicator was parsed and written to CSV but never displayed on screen or announced by a screen reader, so a live reading gave no hazard cue. It now appears first among the status badges, in the error colour, and is spoken ahead of the other flags.
- **CSV exports are numeric again for negative readings** — the UT61E+ sends the minus sign separated from the digits, and that space went straight into the `value` column, so spreadsheets and `pandas.read_csv` typed the whole column as text. Affects both `dmm-cli read --format csv` and the GUI's Export CSV.
- **Session Min/Max/Avg reset when the meter changes mode** — turning the dial from volts to ohms kept the old readings and relabelled them with the new unit, so the stats panel showed figures like "Min: 5.0000 kΩ" from a voltage measurement.
- **The GUI no longer claims to be connected after acquisition dies** — if the background thread stopped unexpectedly the status stayed green and the controls stayed enabled while no reading would ever arrive again. It now drops to disconnected and says so.
...
- **`capture` offers the freeform pass again** — it stopped running once protocols began declaring their own steps, so a capture never reached the stage where you describe a mode the step list doesn't cover. `--list-steps` now lists the selected device's real step IDs instead of a stale internal list, and a `--steps` ID that matches nothing is an error rather than an empty report announced as "Capture complete!".
