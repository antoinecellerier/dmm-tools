...
- **Recording samples keep the `Measurement` whole.** The GUI's in-memory recording buffer used to copy each `Measurement` into a parallel `Sample` struct with five owned `String` fields for mode/unit/range/value/flags. Over a 14-hour recording that was ~2 million redundant heap allocations for what were almost always `Cow::Borrowed` static-lookup-table strings. `Sample` now just carries the `Measurement` and the wall-clock timestamp; display-panel and CSV-export paths use accessor methods (`mode()`, `unit()`, `value_str()`, `flags_str()`) that read directly from the measurement. No more parallel representations to drift between recording and the rest of the pipeline.
